function [x y] = locatie(loc12, loc13, loc14, loc23, loc24, loc34, a)
...
if eon == 1
    x1e = intersect(loc13, loc12, 'rows');
    x2e = intersect(loc13, loc23, 'rows');
    x3e = intersect(loc13, loc14, 'rows');
    x4e = intersect(loc13, loc24, 'rows');
    x5e = intersect(loc13, loc34, 'rows');

    xex = [x1e(:,1); x2e(:,1); x3e(:,1); x4e(:,1); x5e(:,1)];
    xey = [x1e(:,2); x2e(:,2); x3e(:,2); x4e(:,2); x5e(:,2)];
    xex(xex == 0) = [];
    xey(xey == 0) = [];
...
if fon == 1
    x1f = intersect(loc13, loc12, 'rows');
    x2f = intersect(loc13, loc23, 'rows');
    x3f = intersect(loc13, loc14, 'rows');
    x4f = intersect(loc13, loc24, 'rows');
    x5f = intersect(loc13, loc34, 'rows');

    xfx = [x1f(:,1); x2f(:,1); x3f(:,1); x4f(:,1); x5f(:,1)];
    xfy = [x1f(:,2); x2f(:,2); x3f(:,2); x4f(:,2); x5f(:,2)];
    xfx(xfx == 0) = [];
    xfy(xfy == 0) = [];
